...
5. **加工**：加工坊按「生产工单」批量排产（面粉/番茄汁/奶酪等），每批耗时 1~2 个游戏天，
   ...
   升级加工坊可扩容队列并解锁更多配方
6. **灌溉**：花金币建造蓄水池（🪙150，容量 300）、铺设水渠（🪙15/段）并接入地块；
   水渠把蓄水池的水引向四邻耕地，同一连通网络共享水量，每日结算按地块优先级（高/中/低）
   分配有限水量、自动浇水；降雨/暴雨为蓄水池补水，干旱/酷暑加速蒸发；
   设施可停用（断渠断流）与拆除（返还 50%），重新连通或补水后次日自动恢复供水，
   断流/恢复均记入灌溉日志
7. **季节**：春夏秋冬 28 天一轮，冬季光照下降
8. **天气与防灾**：按季节概率生成天气事件（暴雨/干旱/霜冻/暴雪等，持续 1~3 天）并持久化；
...
`player` `plots` `crops` `inventory` `buildings` `animals` `weather_events` `weather_log` `production_jobs` `reservoirs` `canals` `irrigation_log`
